...
namespace DAL.MyDataInitializer
{
    public class MyDataInitializer
    {
        ...
        public void Initialize()
        {
            ...
            logger.LogInformation("Инициализация актов продаж");
            var testSale1 = new Sale {
                Buyer = testBuyer1,
                SalesDate = System.DateTime.Now,
                SalesPoint = testSalesPoint1,
                SalesData = new List<SaleData>
                {
                    new SaleData {Product = testProduct1,Quantity = 3},
                    new SaleData {Product = testProduct2,Quantity = 10}
                }
            };

            var testSale2 = new Sale {
                Buyer = testBuyer1,
                SalesDate = System.DateTime.Now,
                SalesPoint = testSalesPoint2,
                SalesData = new List<SaleData>
                {
                    new SaleData {Product = testProduct2,Quantity = 5},
                    new SaleData {Product = testProduct3,Quantity = 6}
                }
            };

            var testSale3 = new Sale {
                Buyer = testBuyer2,
                SalesDate = System.DateTime.Now,
                SalesPoint = testSalesPoint1,
                SalesData = new List<SaleData>
                {
                    new SaleData {Product = testProduct1,Quantity = 7},
                    new SaleData {Product = testProduct3,Quantity = 8}
                }
            };
            var testSale4 = new Sale {
                Buyer = testBuyer2,
                SalesDate = System.DateTime.Now,
                SalesPoint = testSalesPoint2,
                SalesData = new List<SaleData>
                {
                    new SaleData {Product = testProduct2,Quantity = 10},
                    new SaleData {Product = testProduct3,Quantity = 11}
                }
            };

            salesRepo.Add(new List<Sale> { testSale1, testSale2, testSale3, testSale4 });
            logger.LogInformation("Инициализация актов продаж завершена");
